...
begin
  ...
  sgContactDisplay.ColCount := 6; // Including hidden Id column
  sgContactDisplay.RowCount := 1;

  // Setup visible headers
  ...
  // Hide Id column
  sgContactDisplay.ColWidths[5] := 0;
  sgContactDisplay.Cells[5, 0] := ''; // Optional since it's hidden anyway
...
begin
  ...
  try
    ...
    try
      Response := HttpClient.Post(FApiUrl + 'person/query', ContentStream, nil, Headers);
...
begin
  ...
  try
    ...
    try
      ...
      try
        Response := HttpClient.Post(FApiUrl + 'person/bulk', ContentStream, nil, Headers);
  ...
  try
    ...
    try
      ...
      try
        ...
        Response := HttpClient.Post(FApiUrl + 'person', ContentStream, nil, Headers);
...
var
  ...
  i: Integer;
begin
  // Exclude the hidden column from RemainingWidth and ColCount
  RemainingWidth := sgContactDisplay.ClientWidth - sgContactDisplay.ColWidths[0] - 2;
  EqualWidth := RemainingWidth div (sgContactDisplay.ColCount - 2); // -2: one for No., one for hidden Id

  // Resize only columns 1 to 4
  for i := 1 to 4 do
    sgContactDisplay.ColWidths[i] := EqualWidth;

  // Hide the Id column again just in case
  sgContactDisplay.ColWidths[5] := 0;
end;
